...
export const AuthForm: React.FC<AuthFormProps> = ({
  ...
  const authFormData =
    typePage == "login" ? LoginFormData : RegistrationFormData;
  return (
    <View>
      ...
      {typePage == "registration" && stateRegister == 0 && (
        <ItemButton title="next" handleSubmit={handleSubmit(handlerNextPage)} />
      )}
      {typePage == "registration" && stateRegister > 0 && (
        <View>
          <ItemButton title="back" handleSubmit={handlerPreviousPage} />
        </View>
      )}

      {typePage != "registration" && (
        <ItemButton title="Log in" handleSubmit={handleSubmit(onSubmit)} />
      )}
    </View>
  );
};
